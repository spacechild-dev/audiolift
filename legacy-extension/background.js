...
chrome.commands.onCommand.addListener(async (command) => {
  if (command === 'toggle-audiolift') {
    ...
    const currentSettings = {
      enabled: true, // Default to true fallback
...
chrome.runtime.onInstalled.addListener(async (details) => {
  if (details.reason === 'install') {
    ...
    // IMPROVEMENT: Await the storage set to ensure settings exist before injection
    await chrome.storage.local.set({
      globalSettings: {
        enabled: true, // IMPROVEMENT: Enable by default so user sees immediate effect
        preamp: 0,
        eq32: 0,
        eq64: 0,
        eq125: 0,
  ...
  // IMPROVEMENT: Wait for this to complete
  ...
  try {
    ...
    for (const tab of tabs) {
      ...
      try {
        ...
        // Ignore errors (e.g. script already exists, or page is restricted)
      }
    }
  } catch (e) {
...
// Call injection on startup (when service worker wakes up)
